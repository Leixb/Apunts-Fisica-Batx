...
\newsavebox\madeby
\savebox\madeby{\tikz[color=black!50,opacity=0.2]\node{Created by Leix\_b};}
\newsavebox\distribute
\savebox\distribute{\tikz[color=black!50,opacity=0.2]\node{Do not distribute};}
%\newwatermark*[allpages,color=black!20,angle=60,scale=6,xpos=-20,ypos=15]{\usebox\distribute}
%\newwatermark*[allpages,color=black!20,angle=60,scale=6,xpos=-40,ypos=25]{\usebox\madeby}
...
\begin{center}
    $\sum \vec{F} = 0 \iff$ objecte en MRU o repòs.
\end{center}
...
\begin{align*}
    ...
    \left|F_{a \rightarrow b}\right| &= \left|F_{b \rightarrow a}\right|
\end{align*}
...
Una politja de la qual pengen dues masses $m_1 > m_2$:
\begin{equation}
...
\begin{equation}
    E_\text{perduda} = \frac{v_{f_1}-v_{f_2}}{v_{i_1}-v_{i_2}}
\end{equation}
...
\begin{equation}
    \varepsilon_0 = \SI[input-digits=0123456789\dots]{8.854187817\dots e-12}{\farad\per\metre} \qquad \text{(Permitivitat elèctrica del buit)}
\end{equation}
...
L'energia dissipada per l'efecte Joule ($Q$) ve determinada per:
...
\begin{figure}[H]
    ...
    \begin{center}
        \begin{circuitikz}
            ...
            %\draw (4,0) to[resistor,l=$R_3$] (0,0);
...
\begin{equation}
    \frac{n_1}{n_2} = \frac{\sin \hat{r}}{\sin \ihat} = \frac{v_2}{v_1}
\end{equation}
...
\begin{align*}
    ...
    \varepsilon_0 &= \SI[input-digits=0123456789\dots]{8.854187817\dots e-12}{\farad\per\metre} \qquad &\text{(Permitivitat del buit)}\\
...
\end{align*}
\footnotetext{A la superfície de la Terra i a nivell de mar}
